...
@WebServlet(name = "CadastroHotelServlet", urlPatterns = {"/CadastroHotelServlet"})

public class CadastroHotelServlet extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException, IllegalAccessException, InvocationTargetException {
        request.setCharacterEncoding("UTF-8");
        CadastroHotelFormBean npfb = new CadastroHotelFormBean();
        BeanUtils.populate(npfb, request.getParameterMap());
  request.getSession().setAttribute("CadastroHotelServlet", npfb);
  List<String> mensagens = npfb.validar();
  if (mensagens == null) {
      request.getRequestDispatcher("ConfirmarCadastroHotel.jsp").forward(request, response);
  } else {

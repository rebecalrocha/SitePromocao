...
@WebServlet(name = "CadastroSiteServlet", urlPatterns = {"/CadastroSiteServlet"})
public class CadastroSiteServlet extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException, IllegalAccessException, InvocationTargetException {
        request.setCharacterEncoding("UTF-8");
        CadastroSiteFormBean npfb = new CadastroSiteFormBean();
        BeanUtils.populate(npfb, request.getParameterMap());
  request.getSession().setAttribute("CadastroSiteServlet", npfb);
  List<String> mensagens = npfb.validar();
  if (mensagens == null) {
      request.getRequestDispatcher("ConfirmarCadastroSite.jsp").forward(request, response);
  } else {
